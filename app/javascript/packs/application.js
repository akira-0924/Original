...
$(function() {
	$("section input").on("click", function(){
			$(".wizard > .actions a[href='#next']").fadeIn();
	});
	...
	$(".q_all3").on("click", function(){
		var q_Num31 = $(".type56:checked").length,
			q_Num32 = $(".type78:checked").length;
			if(q_Num31>0 && q_Num32>0){
			$(".wizard > .actions a[href='#finish']").fadeIn();
			}
	});
...
$(function() {
	$("#drawer-check").on("click", function(){
			$("#DR00").toggleClass('drawer-item00');
...
$(function() {
	$("#outputWrapper").on("hover", function(){
			$(".aaaa").after('');
	});
});
$(function(){
	$('.YouTube').each(function() {
	  ...
	  iframe.before('<img src="http://img.youtube.com/vi/'+id+'/mqdefault.jpg" class="youtube" size: "370x270" width="370px" height="180px" />').remove();
  ...
  $(function () {
	...
	        });
	    });
	$("#arrowSC").on("click", function(){
		var trans = $("#histry_st").offset().top;
		$("html,body").animate({
			scrollTop : trans
		},1000);
	});
...
$(function () {
	$( '#upload_icon' ).on("change", function () {
		...
		$( '#outputWrapper' ).empty();
		$( '#outputWrapper' ).append( '<img src="' + loadedImageUri + '" class="pre1" width="120px" height="120px">' )
		};
	...
	$( '#post_pre' ).on("change", function () {
		...
		var loadedImageUri = event.target.result;
		$( '#POST_PREVIEW' ).empty();
		$( '#POST_PREVIEW' ).append( '<img src="' + loadedImageUri + '" class="pre1 post_pre1" width="270px" height="270px">' )
		};
		fileReader.readAsDataURL( selectedFile );
	});
});
